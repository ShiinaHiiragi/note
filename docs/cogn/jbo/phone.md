...
2. 逻辑语的常用术语
    ...
    2. 句（jufra）：最基本的句子是谓句（bridi），包括谓语（selbri）与项（sumti）两个部分
...
1. 字符表与发音
    ...
    !!! note "字母的发音"
        1. 「BY*」类：元音字母的读法是在其后加上「bu」，例如「y」的读法是「.ybu」
        2. 「BY2」类：辅音字母的读法是在其后加上「y.」，例如「c」的读法是「cy.」。特别地，辅音字母「'」的读法为「.y'y」

        字符「h」「q」「w」的读法分别为「.y'y.bu」「ky.bu」「vy.bu」
