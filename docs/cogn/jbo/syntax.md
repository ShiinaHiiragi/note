...
3. 时态：任何关于时间的结构词或结构词序列都可以放在谓词之前，使整个谓句进入相应时态
...
2. 用「nu」描述事件发生的时间，用「SE」类词换位。也可直接使用「ti'u」或「di'e」指定动作发生的时间
    ...
    !!! note ""
        ...
        - lenu mi jbena[^4] cu se detri li pamu pi'e bi
...
3. 小时、星期与月份
    ...
    2. 使用表示星期的根词「djedi[^8]」组成的专名表示某星期
    ...
    3. 使用表示月份的根词「masti[^9]」组成的专名表示某星期
    ...
    4. 使用表示年份的根词「nanca[^10]」组成的专名表示某一年，例如用「la renonopanac.」表示二〇〇一年
...
[^4]: jbena：$x_1$ 为 $x_2$ 所生，日期是 $x_3$，地点是 $x_4$
[^5]: clira：$x_1$（事件/状态）较早，基于标准 $x_2$
[^6]: lerci：$x_1$（事件/状态）较晚，基于标准 $x_2$
[^7]: cacra：$x_1$（事件/状态）的持续时间是 $x_2$ 小时（默认为 $1$ 小时），基于标准 $x_3$
[^8]: djedi：$x_1$（事件/状态）的持续时间是 $x_2$ 天（默认为 $1$ 天），基于标准 $x_3$
[^9]: masti：$x_1$（事件/状态）的持续时间是 $x_2$ 月（默认为 $1$ 月），基于标准 $x_3$
[^10]: nanca：$x_1$（事件/状态）的持续时间是 $x_2$ 年（默认为 $1$ 年），基于标准 $x_3$
