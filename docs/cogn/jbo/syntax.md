...
### 3.2.2 时间
1. 时刻：用「tcika[^3]」描述时刻时，默认使用二十四小时制，「pi'e」分隔顺序依次为时、分、秒
...
2. 日期：用「detri[^4]」描述日期时，「pi'e」分隔顺序依次为日、月、年
...
[^3]: tcika：$x_1$ 是 $x_2$ 的时刻，日期是 $x_3$，位置在 $x_4$，按照历法 $x_5$
[^4]: detri：$x_1$ 是 $x_2$ 的日期，位置在 $x_3$，按照历法 $x_4$
