...
5. 直言三段论：两个包含共同项的直言命题作为前提推出一个直言命题为结论
    ...
    2. 三段论的公理与规则：一类对象的全体具有或不具有某个性质，那么这类对象的一部分也具有或不具有同样的性质
        ...
        2. 中项在前提中必须周延至少一次：若中项中前提未被断定过其全部外延，则大小项都只与中项的一部分外延发生联系
    ...
    3. 三段论的格与式
        ...
        <figure markdown>
            ![](../assets/lattice.svg)
            <style> img[src$="lattice.svg"] { width: 500px; } </style>
        </figure>
...
6. 其他三段论
    1. 复合三段论：由两个或以上的三段论构成的特殊的三段论形式  
        前进式（后退式）复合三段论：以前一个三段论的结论作为后一个三段论的大前提（小前提）的三段论
...
2. 选言命题：断定事物若干种可能情况的命题
    1. 相容选言命题：断定事物若干可能情况中至少有一种情况存在的命题，记作 $p \vee q$，读作 「$p$ 析取 $q$」
        1. 否定一部分选言肢，就要肯定另一部分选言肢
        2. 肯定一部分选言肢，不能否定另一部分选言肢
    2. 不相容选言命题：断定事物若干可能情况中有只有一种情况存在的命题，记作 $p \dot{\vee} q=(p\vee q)\wedge \overline{(p \wedge q)}$，读作 「$p$ 强析取 $q$」
        1. 否定肯定式：肯定一个选言肢，就要否定其余的选言肢
        2. 肯定否定式：否定一个选言肢以外的选言肢，就要肯定未被否定的那个选言肢
3. 假言命题：断定事物情况之间条件关系的命题
    1. 前件表示条件的肢命题；后件表示依赖该条件而成立的命题
...
2. 假言选言推理的分类
    1. 简单构成式：前提肯定两个假言命题的不同前件，结论肯定两个假言命题的相同后件，即由 $p\to q, r\to q, p\vee r$ 推出 $q$
    2. 简单破坏式：前提否定两个假言命题的不同后件，结论否定两个假言命题的相同前件，即由 $p\to q, p\to r, \overline{q}\vee \overline{r}$ 推出 $\overline{p}$
    3. 复杂构成式：前提肯定两个不同假言命题的两个不同的前件，结论肯定两个不同的后件，即由 $p\to r, q\to s, p\vee q$ 推出 $r\vee s$
    4. 复杂破坏式：前提否定两个不同假言命题的两个不同的后件，结论否定两个不同的前件，即由 $p\to q, r\to s, \overline{r}\vee \overline{s}$ 推出 $\overline{p}\vee \overline{r}$
...
4. 规范三段论：直言三段论中引入规范模态词的一种三段论推理。一般大前提是规范命题，小前提是直言命题，结论是规范命题
