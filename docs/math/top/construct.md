...
3. 一致收敛度量与一致收敛拓扑
    1. 一致收敛度量：设 $X$ 是一个集合，$(Y, \rho)$ 是一个度量空间．记 $Y^X$ 为从 $X$ 到 $Y$ 的所有映射的集合，定义 $\widetilde{\rho}: Y^X\times Y^X \to R$ 使得对于任何 $f, g\in Y^X$ 有 $\widetilde{\rho}(f, g) = \left\{\begin{aligned}&1, &\ \exists x\in X: \rho(f(x), g(x))\geqslant 1 \\& \sup\{\rho(f(x), g(x)) | x\in X\}, &\ \textsf{否则}\end{aligned}\right.$，则容易验证 $\widetilde{\rho}$ 是 $Y^X$ 的一个度量，称之为 $Y^X$ 的一致收敛度量；度量空间 $(Y^X, \widetilde{\rho})$ 称为映射空间（一致收敛度量）
