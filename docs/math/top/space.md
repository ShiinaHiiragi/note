...
4. 开集：设 $A$ 是度量空间 $X$ 的一个子集，若 $\forall a\in A\ \exists \varepsilon >0: B(a, \varepsilon)\subset A$，则称 $A$ 是度量空间 $X$ 中的一个开集
    ...
    2. 度量空间 $X$ 中的开集具有以下性质
        ...
        2. 任意两个开集的交是一个开集
        3. 任意一个开集族的并是一个开集
5. 邻域：设 $x$ 是度量空间 $X$ 的一个点，$U$ 是 $X$ 的一个子集，若存在一个开集 $V$ 满足条件 $x\in V\subset U$，则称 $U$ 是点 $x$ 的一个邻域

    !!! note "邻域定义的等价陈述"
        设 $x$ 是度量空间 $X$ 的一个点，则 $X$ 的子集 $U$ 是 $x$ 的一个邻域的充要条件是 $x$ 有某一个球形邻域包含于 $U$

6. 连续映射
    1. 连续：设 $X$ 和 $Y$ 是两个度量空间，$f: X\to Y, x_0\in X$．若对于 $f(x_0)$ 的任何一个球形邻域 $B(f(x_0), \varepsilon)$ 存在 $x_0$ 的某一个球形邻域 $B(x_0, \delta)$ 使得 $f(B(x_0, \delta))\subset B(f(x_0), \varepsilon)$，则称映射在点 $x_0$ 处是连续的
...
## 1.2 拓扑空间
### 1.2.1 拓扑空间
1. 拓扑：设 $X$ 是一个集合，$\mathscr T\subset \mathcal P(X)$，若 $\mathscr T$ 满足以下条件，则称 $\mathscr T$ 是 $X$ 的一个拓扑
...
4. 邻域：设 $(X, \mathscr T)$ 是一个拓扑空间，$x\in X$．若 $U$ 是 $X$ 的一个子集，且存在一个开集 $V\in \mathscr T$ 使得 $x\in V\subset U$，则称 $U$ 是点 $x$ 的一个邻域．
...
5. 内部：设 $X$ 是一个拓扑空间，$A\subset X$，若 $A$ 是点 $x\in X$ 的一个邻域，则称点 $x$ 是集合 $A$ 的一个内点，集合 $A$ 的所有内点构成的集合称为集合 $A$ 的内部，记作 $A^o$
