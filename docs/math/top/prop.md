...
4. 道路连通分支
    ...
    3. $n$ 维 $\text{Euclid}$ 空间 $R^n$ 的任何一个连通开集都是道路连通的，且 $R^n$ 中任何开集的每一个道路连通分支同时也是它的一个连通分支
...
2. $\text{Lindelof}$ 空间：设 $X$ 是一个拓扑空间，若 $X$ 的每一个开覆盖都有一个可数子覆盖，则称拓扑空间 $X$ 是一个 $\text{Lindelof}$ 空间
    1. 包含不可数个点的离散空间不是一个 $\text{Lindelof}$ 空间
...
2. $\text{Urysohn}$ 嵌入定理：每一个满足第二可数性公理的 $\text T_3$ 空间都同胚于 $\text{Hilbert}$ 空间 $H$ 的某一个子空间
    1. $\text{Hilbert}$ 空间 $H$ 是一个可分空间，且 $H$ 满足第二可数性公理，因此 $H$ 也是一个 $\text{Lindelof}$ 空间
    2. 设 $X$ 是一个拓扑空间，则下列条件等价
...
3. 设 $\{X_i\}_{i\in Z_+}$ 是可度量化空间的一个可数族，则积空间 $\prod_{i\in Z_+}X_i$ 是一个可度量化空间
...
1. 可数紧空间：设 $X$ 是一个拓扑空间，若 $X$ 的每一个可数开覆盖都有有限子覆盖，则称拓扑空间 $X$ 是一个可数紧空间
    1. 每一个紧空间都是可数紧空间
    2. 每一个 $\text{Lindelof}$ 的可数紧空间都是紧空间
2. 列紧空间：设 $X$ 是一个拓扑空间，若 $X$ 的每一个无限子集都有凝聚点，则称拓扑空间 $X$ 是一个列紧空间
    1. 每一个可数紧空间都是列紧空间
    2. 下降序列：设 $\{A_i\}_{i\in Z_+}$ 是一个由集合构成的序列，如果它满足条件 $A_i\supset A_{i+1}$ 对于每一个 $i\in Z_+$ 成立，则称序列 $\{A_i\}_{i\in Z_+}$ 是一个下降序列；在某一个拓扑空间中的一个由非空闭集构成的下降序列也称作一个非空闭集下降序列
    3. 设 $X$ 是一个拓扑空间，则拓扑空间 $X$ 是一个可数的紧空间当且仅当 $X$ 中任何一个非空闭集下降序列 $\{A_i\}_{i\in Z_+}$ 有 $\bigcap_{i\in Z_+}A_+\neq \varnothing$
...
5. 仿紧空间：设 $X$ 是一个拓扑空间，如果 $X$ 的每一个开覆盖都有一个局部有限的开覆盖是它的加细，则称拓扑空间 $X$ 是一个仿紧空间
    ...
    3. 设 $X$ 是一个满足第二可数性公理的局部紧 $\text T_2$ 空间，则 $X$ 有一个开覆盖 $\{V_1, V_2, \cdots\}$ 满足条件： 对于每一个 $i\in Z_+$，闭包 $V_i^-$ 是一个包含于 $V_{i+1}$ 的紧子集
