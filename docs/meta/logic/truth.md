...
2. 句法悖论的解决方案
    ...
    2. 公理集合论：对于 $\text{Russell}$ 悖论
        1. $\mathrm{ZFC}$ 公理集合论否定任一性质 $R$ 决定一个集合 $S$，从而对集合的生成做出更严格的限制
        2. $\mathrm{BGC}$ 公理集合论否定对任一集合 $S$，$S \in S$ 是一有意义的命题，从而对集合元素的身份做出更严格的限制
...
### 2.2.3 语义悖论
1. 语义悖论例举
    1. 说谎者悖论：起源于公元前六世纪，希腊克里特岛人 $\text{Epimennides}$ 说的「所有的克里特岛人都说谎」。一般表述为 $\boxed{\textsf{本方框内的这个语句是假的}}$
    ...
    4. $\text{Grelling}$ 悖论：也称非自谓悖论。将所有形容西分为两类：对自身适用的（例如「四个字的」或「中文的」），称之为自谓的；对自身不适用的（例如「单音节的」或「红色的」），称之为非自谓的。则形容词「非自谓的」是自谓的当且仅当它是非自谓的
