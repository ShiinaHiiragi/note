    ...
    2. 人类以语言为基础，以思维和文化为特征的高阶认知能力是由神经和心理的初阶认知能力进化而来的
        ...
        !!! note "$\text{Sapir}-\text{Whorf}$ 假说"
            ...
            3. $\text{Sapir}-\text{Whorf}$ 假说的重要推论
                1. 语言结构影响人们对现实的认知结构（或者更为强式的表达是人们以与其语言结构相结合的结构来认识现实）
                2. 语言以不同方式切分现实，但不存在切分世界的自然方式
                3. 这种语言上的差异性是隐蔽或无意识的
                4. 语言结构对人类认知的限制亦是无意识的，因此不同语言的观察者对同意现象的认知图像是不同的
...
2. $\text{Chomsky}$ 的转换生成语法
    1. 转换生成语法形成时期：20 世纪 50 年代中期到 70 年代中期，主要成就是转换语法（$\text{TG}$）
        1. 句法结构理论（$\text{SS}$）：$\text{Chomsky}$ 主张把语义排除在语法之外
            1. 短语结构规则：也称作重写规则，它试图用有限的规则来生成无限的语句。重写规则通过形式化方法和递归定义，生成一系列的短语结构
            2. 转换规则：由重写规则生成一系列的短语规则，可分为词汇前结构和词汇后结构。前者由非终极符构成，称为深层结构，后者由终极符构成，称为表层结构语法
            3. 形态音位规则：转换规则将深层结构的逻辑语法关系映射为表层结构的语言关系与语音关系
        2. 标准理论（$\text{ST}$）：包括深层结构与表层结构，深层结构即语义部分和语义规则，表现语句的语义；表层结构即语音部分和语音规则，表现语句的语言
            1. 扩展的标准理论（$\text{EST}$）：虽然语义主要是由深层结构决定，但表层结构对语义解释也起一定的作用
            2. 修正扩展的标准理论（$\text{REST}$）：将语义解释放到表层结构上，时期理论结构增加一个与语音表现并列的新层级，即逻辑形式表现
...
3. 普遍语法（$\text{UG}$）：说者知道一套适用于所有语言的原则即一些明确限定的参数，这些参数在不同的语言之间是各不相同的。语言的习得意味着如何将这些原则应用于某种特殊的语言，并对每一种参数了解它的值是否合适
    1. 语言能力：人类在句法和语义方面拥有的能力
        1. 能造出和理解数量无限的语句，其中包括从来没有听到过的语句
        2. 能辨认出哪些语句是合法的、可以被接受的
        3. 能辨认语句的结构
        4. 能辨别歧义、同义或释义现象
...
## 3.3 语义与意义理解
1. 真值函数语义学
    1. 形式逻辑的真值语义：命题或语句的逻辑意义是真与假，直言命题的真值由主谓项间在现实世界或可能世界中的关系确定
...
3. 形式语义学
    1. 范畴语法：$\text{Frege}$ 建立语言表达式的范畴与它所指称的现实世界中的实体类型之间的相关性
        ...
        2. 对每一个逻辑类型、该类型的一个表达式所确定类型的实体作为其指称
            1. 个体词项指称现实世界中的实体
            2. 一元谓词指称从个体到真值的函数；一元或二元逻辑联结词的指称也是一个真值函数
            3. 量词的指称是领域中个体的数量
            4. 陈述语句的指称是某种真值
    2. $\text{Montague}$ 语法：创建类似逻辑形式系统的英语部分语句系统，基于句法范畴和语义类型的对应和意义组合原则，给出语句系统的语义解释
        1. 类型$-$逻辑语法：更贯彻意义的组合原则，对自然语言的词条指派作为语义表现的 $\lambda-$词项和作为句法层面的范畴，依据间接的规则逐层并行推演，最后获得语句的范畴类型值  
            范畴的语法逻辑：将模态逻辑方法应用到自然语言领域
        2. 广义量词理论：从 $\text{Montague}$ 对自然语言量化句的研究获得启发，将量词的概念推广，深入说明自然语言的限定词和名词短语的量化意义及其语义共性，用推广的两次概念表述数学理论
        3. 话语表现理论：处理语句之间名词与代词的照应关系以及动词在时间方面的联系，对语句序列的语义分析采用渐进递增的动态方法，确立关于自然语言语义表现的推演系统  
