...
2. 二次曲面方程与不变量：由 $F(x, y, z)$ 的系数组成的一个非常数函数 $f$，若经过 $\text{Euclid}$ 变换
    ...
    时有 $f\left(a_{11}, a_{12}, \cdots, a_{44}\right)=f\left(a_{11}^{\prime}, a_{12}^{\prime}, \cdots, a_{44}^{\prime}\right)$，则这个函数 $f$ 称作二次曲面在 $\text{Euclid}$ 变换下的不变量；若函数 $f$ 的值只是经过旋转变换
    ...
    不变，则这个函数称作二次曲面在 $\text{Euclid}$ 变换下的半不变量
