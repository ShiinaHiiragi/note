...
## 3.1 连通性
### 3.1.1 连通空间
1. 隔离：设 $A, B$ 是拓扑空间 $X$ 的两个自己，若 $(A\cap \overline B)\cup (B\cap \overline A) =\varnothing$，则称子集 $A$ 和 $B$ 是隔离的，此时 $A$ 与 $B$ 无交且其中的任何一个不包含另一个的任何聚点
...
### 3.3.2 可度量化空间
1. 设 $(X, \mathscr T)$ 是拓扑空间，若存在 $X$ 的一个度量 $\rho$ 使得拓扑 $\mathscr T$ 即是由度量 $\rho$ 诱导的拓扑 $\mathscr T_\rho$，则称 $(X, \mathscr T)$ 是一个可度量化空间
...
2. 列紧空间：设 $X$ 是一个拓扑空间，若 $X$ 的每一个无限子集都有聚点，则称拓扑空间 $X$ 是一个列紧空间
    1. 每一个可数紧空间都是列紧空间
