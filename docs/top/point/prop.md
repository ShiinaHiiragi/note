...
5. 细分：设集族 $\mathscr {A, B}$ 都是集合 $X$ 的覆盖，如果 $\mathscr A$ 中的每一个元素都包含于 $\mathscr B$ 的某一个元素之中，则称 $\mathscr A$ 是 $\mathscr B$ 的一个细分
...
5. 仿紧空间：设 $X$ 是一个拓扑空间，若 $X$ 的每个开覆盖都有一个局部有限的开覆盖是其细分，则称拓扑空间 $X$ 是一个仿紧空间
    1. 每一个仿紧的正则空间都是正规空间
