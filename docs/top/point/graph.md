...
5. 子图与补图
    1. 子图：设图 $G = (V, E)$，若在图 $G' = (V', E')$ 有 $V' \subseteq V, E' \subseteq E$ 且 $E'$ 中边所关联的顶点在 $V'$ 中，则称 $G'$ 为 $G$ 的子图
        ...
        2. 导出子图：若 $V' \subseteq V$，以 $V'$ 为顶点集，端点均在 $V"$ 中的所有边的全体为边集，构成的子图称为由 $V'$ 导出的 $G$ 的子图，又称为 $G$ 的导出子图，记为 $G(V')$
            ...
            !!! note "团"
                若图 $V$ 的一个导出子图 $G(V')$ 是完全图，则称该图为团，定义 $\omega(G)$ 为 $G$ 中最大团的顶点数
...
6. 路与回路：对于图 $G = (V, E)$ 的一个有限点边交替序列 $r = \left<v_0, e_1, v_1, e_2, \cdots, e_m, v_m\right>$
    ...
    3. 路：顶点 $v_0, v_1, \cdots, v_m$ 互不相同的链
        ...
        2. 距离：在 $G = (V, E, \omega)$ 中，路 $p$ 上的所有边带的权之和称为路 $p$ 的长度，记为 $\omega(p)$．顶点 $u, v$ 之间的最短路长度称为 $u, v$ 之间的距离，记为 $d(u, v)$
        3. 若图 $G$ 的每个顶点度数至少为 $2$，则 $G$ 包含一条回路

    以上概念均可定义有向图的对应概念

7. 秩与零度：设图 $G$ 有 $n$ 个顶点，$e$ 条边与 $\omega$ 个分支，称 $n - \omega$ 为图的秩，$e - n + \omega$ 为图的零度
8. 剖分：设 $G$ 是一个给定的图，对 $G$ 实行有限次「删去一条边 $\{u, v\}$ 后添加一个新顶点 $w$ 以及新边 $\{u, w\}$ 与 $\{v, w\}$」的过程称为 $G$ 的一个剖分
...
### 4.1.2 图着色
1. 点着色：设 $G$ 是一个无自环的图，对 $G$ 的每个顶点着色，使得没有两个相邻的顶点着上相同的颜色，这种着色称为图的正常着色．图 $G$ 的顶点可用 $k$ 中颜色正常着色，则称 $G$ 为 $k-$可着色的，使 $G$ 是 $k-$可着色的最小 $k$ 称为 $G$ 的色数，记为 $\chi(G)$，并称 $G$ 是 $\chi(G)$ 色的
    1. 特定图的色数
        1. $G$ 是零图当且仅当 $\chi(G) = 1$
        2. 对于完全图 $K_n$ 有 $\chi(K_n) = n$ 且 $\chi(\overline{K_n}) = 1$
        3. 对于 $n$ 个顶点构成的回路 $G$，当 $n$ 是偶数时 $\chi(G_n) = 2$；当 $n$ 是奇数时 $\chi(G_n) = 3$
        4. $G$ 是二分图当且仅当 $\chi(G) = 2$
    2. 如果图 $G$ 顶点的最大度数为 $\Delta(G)$，则 $\chi(G) \leqslant 1 + \Delta(G)$
    3. $\text{Brooks}$ 定理：如果连通图 $G$ 的顶点最大度数为 $\Delta(G)$，$G$ 不是奇回路，又不是完全图，则 $\chi(G) \leqslant \Delta(G)$
2. 边着色：设 $G$ 是没有自环的图，若对 $G$ 的边着色，使得没有两条相邻的边着上相同的颜色，这种着色称为正常边着色．$G$ 可用 $k$ 中颜色正常边着色，则称 $G$ 是 $k-$边可着色的．使 $G$ 的 $k-$边可着色的最小 $k$ 称为 $G$ 的边色数，记为 $x'(G)$，并称 $G$ 是 $x'(G)$ 边色的
    1. $\text{Vizing}$ 定理：设 $G$ 是一个简单图，其顶点最大度数是 $\Delta(G)$，则 $x'(G) = \Delta(G)$ 或者 $x'(G) = \Delta(G) + 1$
...
### 4.1.4 图的实例
1. 二分图：若图 $G$ 的顶点集 $V$ 能划分为两个子集 $V_1$ 与 $V_2$，且每条边的一个端点在 $V_1$ 中，另一个端点在 $V_2$ 中，则称 $G$ 为二分图，记为 $G(V_1, V_2)$
    1. 完全二分图：若简单图 $G$ 具有划分 $\{V_1, V_2\}$ 且 $V_1$ 中每个顶点与 $V_2$ 中每个顶点恰有一条边相连，则称 $G$ 为完全二分图．若 $|V_1| = m, |V_2| = n$，这样的完全二分图记为 $K_{m, n}$
...
2. $\text{Euler}$ 图：若（有向）图 $G$ 中具有一条包含 $G$ 中所有边的（有向）闭链，则称之为 $\text{Euler}$（有向）闭链，此时称 $G$ 为 $\text{Euler}$（有向）图；若图 $G$ 中具有一条包含 $G$ 中所有边的（有向）开链，则称之为 $\text{Euler}$（有向）开链，此时称 $G$ 为半 $\text{Euler}$（有向）图
    1. 设 $G$ 是连通图，则 $G$ 是 $\text{Euler}$ 图当且仅当 $G$ 的所有顶点都是偶顶点；$G$ 是半 $\text{Euler}$ 图当且仅当 $G$ 中有且仅有两个奇顶点
    2. 设 $G$ 是连通有向图，则 $G$ 是 $\text{Euler}$ 有向图当且仅当 $G$ 的每个顶点 $v$ 都满足 $d^+(v) = d^-(v)$；$G$ 是半 $\text{Euler}$ 有向图当且仅当 $G$ 中有且仅有两个奇顶点，其中一个奇顶点的入度比出度大 $1$，另一个奇顶点的出度比入度大 $1$；其余顶点的出度等于入度
    3. 一笔画问题：如果在一个图中奇顶点个数为 $2k$ 个，则该图是 $k$ 笔画的
3. $\text{Hamilton}$ 图：若（有向）图 $G$ 具有一条包含 $G$ 中所有顶点的（有向）回路，则称该回路为 $\text{Hamilton}$ 回路，此时称 $G$ 为 $\text{Hamilton}$（有向）图；若图 $G$ 具有一条包含 $G$ 中所有顶点的路，则称该回路为 $\text{Hamilton}$ 路，此时称 $G$ 为半 $\text{Hamilton}$（有向）图
    1. 充分条件：若 $G$ 是 $n \ (n \geqslant 3)$ 个顶点的简单图，对于每一对不相邻的顶点 $u, v$ 都有 $d(u) + d(v) \geqslant n$，则 $G$ 是 $\text{Hamilton}$ 图
        1. 设 $G$ 是 $n \ (n \geqslant 3)$ 个顶点的简单图，若对于每一个顶点 $v$ 都有 $d(v) \geqslant \dfrac{n}{2}$，则 $G$ 是 $\text{Hamilton}$ 图
        2. 若 $G$ 是 $n$ 个顶点的简单图，对于每一对不相邻的顶点 $u, v$ 都有 $d(u) + d(v) \geqslant n - 1$，则 $G$ 是半 $\text{Hamilton}$ 图
    2. 必要条件：若图 $G$ 是 $\text{Hamilton}$ 图，则对于顶点集 $V$ 的每一个非空真子集 $S$，$\omega(G - S) \leqslant |S|$ 均成立，其中 $|S|$ 表示 $S$ 的顶点数，$G - S$ 表示 $G$ 中删去顶点子集 $S$ 后得到的图
    3. 竞赛图：每两个顶点之间恰有一条弧的有向图
        1. 任何一个竞赛图是半 $\text{Hamilton}$ 有向图
        2. 任何一个强连通的竞赛图 $G$ 必是 $\text{Hamilton}$ 有向图
        3. $\text{King} - \text{Chicken}$ 定理：在任何竞赛图中，存在最大出度顶点到其他任何顶点的、长度不超过 $2$ 的有向路
4. 平面图：若一个图能画在平面上使其边除了在顶点处之外互不相交，则称该图为平面图，或称该图为能嵌入平面的
    1. 面：平面图 $G$ 嵌入平面后得到图 $\overline G$，将其分成若干个连通闭区域，每个连通闭区域称为 $G$ 的一个面，其中恰有一个无界的面，称为外部面，其余的面称为内部面
    2. $\text{Euler}$ 公式：若连通平面图 $G$ 有 $n$ 个顶点，$e$ 条边与 $f$ 个面，则有 $n - e + f = 2$
        1. 完全图 $K_5$ 与二分图 $K_{3, 3}$ 是非平面图
        2. 在平面简单图 $G$ 中至少存在一个顶点 $v_0$，有 $d(v_0) \leqslant 5$
    3. $\text{Kuratowski}$ 定理：图 $G$ 是平面图当且仅当其任何子图都不是 $K_5$ 或 $K_{3, 3}$ 的剖分
    4. 几何对偶：设 $\overline G$ 是平面图 $G$ 的平面嵌入，则构造 $G^*$
        1. 在 $\overline G$ 的每个面 $f$ 恰放唯一的一个顶点 $f^*$
        2. 对 $\overline G$ 的两个面 $f_i, f_j$ 的公共边 $x_k$ 作边 $x_k^* = \{f_i^*, f_j^*\}$ 与 $x_k$ 相交

        称之为 $G$ 的几何对偶

        1. 设 $G$ 是有 $n$ 个顶点，$e$ 条边与 $f$ 个面的连通平面图，则 $G$ 的几何对偶 $G^*$ 有 $f$ 个顶点，$e$ 条边与 $n$ 个面
        2. $G$ 是连通平面图当且仅当 $G^{**}$ 同构于 $G$ 

## 4.2 树
1. 树的等价定义：设 $T_n$ 有 $n$ 个顶点，$e$ 条边，则以下均为树的等价定义
    1. $T$ 是连通无回路的图
    2. $T$ 是无回路图，且 $e = n - 1$
    3. $T$ 是连通图，且 $e = n - 1$
    4. $T$ 是无回路图，且在 $T$ 的任何两个不相邻的顶点之间添加一边，恰得一条回路
    5. $T$ 是最小连通图，即删去任意一边后就不连通
    6. $T$ 得每一对不同的顶点之间有唯一一条路

    当 $T$ 是树时，定义如下概念：

    1. 叶点与分枝点：树中度数为 $1$ 的顶点称为叶点，也成为悬挂点；度数大于 $1$ 的顶点称为分枝点
    2. 森林：不相交的树全体
    3. 平凡树：即平凡图

2. 树的性质
    1. 若 $G$ 是 $n$ 个顶点，$\omega$ 个分枝点的森林，则 $G$ 有 $n - \omega$ 条边
    2. 在任意一个非平凡树 $T$ 中，至少有两个叶点
3. 有根树
...
2. 割集：设 $D$ 是图 $G$ 的一个边集，若在 $G$ 中删去 $D$ 的全部边后所得的秩减少 $1$，而 $D$ 的任何真子集均无此性质，则称 $D$ 为 $G$ 的割集．
    1. 断集：设图 $G$ 顶点非空子集为 $V' \subseteq V$，在 $G$ 中的一个端点在 $V'$ 中，另一个端点在 $V - V'$ 中的所有边组成的集合称为 $G$ 的一个断集或边割，记为 $E(V' \times \overline{V'})$ 或简记为 $(V', \overline{V'})$
    ...
    3. 基本割集：设 $T$ 是连通图 $G$ 的给定生成树，对于只包含 $T$ 中一条枝的割集，称此割集为关于 $T$ 的基本割集
        1. 连通图 $G$ 有 $e$ 条边与 $n$ 个顶点，给定的生成树 $T$ 有 $n - 1$ 条枝，所以恰有 $n - 1$ 个基本割集，这些割集的全体称为生成树 $T$ 的基本割集组
    ...
    4. 基本回路：设 $T$ 是连通图 $G$ 的给定生成树，在 $T$ 中加一条弦，恰产生一条回路，称此回路为关于 $T$ 的回路
        1. 连通图 $G$ 有 $e$ 条边与 $n$ 个顶点，给定的生成树 $T$ 有 $n - 1$ 条枝与 $e - n + 1$ 条弦，所以恰有 $e - n + 1$ 个基本回路，这些回路的全体称为生成树 $T$ 的基本回路组
