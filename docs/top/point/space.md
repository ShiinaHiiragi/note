...
4. 开集：设 $A$ 是度量空间 $X$ 的一个子集，若 $\forall a\in A\ \exists \varepsilon > 0: B(a, \varepsilon)\subseteq A$，则称 $A$ 是度量空间 $X$ 中的一个开集
    ...
    2. 度量空间 $X$ 中的开集具有以下性质
        ...
        2. 任意两个开集的交是一个开集
        3. 任意一个开集族的并是一个开集
5. 邻域：设 $x$ 是度量空间 $X$ 的一个点且 $U \subseteq X$，若存在一个开集 $V$ 满足条件 $x\in V\subseteq U$，则称 $U$ 是点 $x$ 的一个邻域
    1. 邻域定义的等价陈述：设 $x$ 是度量空间 $X$ 的一个点，则 $X$ 的子集 $U$ 是 $x$ 的一个邻域当且仅当 $x$ 有某个球形邻域包含于 $U$
...
### 1.2.2 闭集与边界
1. 凝聚点与孤立点：设 $X$ 是一个拓扑空间，$A\subseteq X$
...
3. 闭集：设 $X$ 是一个拓扑空间，$A\subseteq X$，若 $d(A) \subseteq A$，则称 $A$ 是拓扑空间 $X$ 的一个闭集
...
5. 内部：设 $X$ 是一个拓扑空间，$A\subseteq X$，若 $A$ 是点 $x\in X$ 的一个邻域，则称点 $x$ 是集合 $A$ 的一个内点，集合 $A$ 的所有内点构成的集合称为集合 $A$ 的内部，记作 $A^o$
