...
2. 超穷序列：定义域为序数 $\alpha$ 的映射称为长度为 $\alpha$ 的序列．不严格地定义映射 $\mathbf{F}: \mathbf{On} \rightarrow \mathbf{V}$，如果存在一个公式 $\varphi(x, y)$ 有 $\forall x \exists ! y \ \varphi(x, y)$，并定义类 $\mathbf{F}=\{(x, y) \mid \varphi(x, y)\}$，则 $\mathbf{F}(x)$ 就是使得 $\varphi(x, y)$ 成立的唯一的 $y$
