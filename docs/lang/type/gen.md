...
1. $\lambda-$项：$\lambda$ 演算的表达式称为 $\lambda-$项．定义变元的无穷集 $V = \left\{u, v, w, \cdots\right\}$，则所有 $\lambda-$项的集合定义 $\Lambda$ 如下
    ...
    1. 子项：设 $P$ 是一个 $\lambda-$项，定义多重集 $\mathrm{Sub}(P)$ 包含 $M$ 的所有子项
        ...
        1. 子项作为关系具有自反性与传递性
        2. 真子项：若 $L$ 是 $M$ 的子项且 $L$ 与 $M$ 不等同，则称 $L$ 是 $M$ 的真子项
...
2. $\alpha-$等价：定义 $\alpha-$转换或称 $\alpha-$等价性 $=_{\alpha}$ 如下
    1. 重命名
...
3. $\beta-$归约：设 $P, Q \in \Lambda_{2}$，定义单步 $P \to_{\beta} Q$ 如下
    1. 基础：$(\lambda x: \sigma.M) N \rightarrow_\beta M[x:=N]$
...
4. $\lambda_2$ 的性质：下列 $\lambda_{\to}$ 的性质在 $\lambda_2$ 中也成立
    1. 自由变元引理
    2. 稀疏化引理
    3. 压缩引理
    4. 生成引理
    ...
    6. 类型唯一性
    7. 替换引理
    8. $\text{Church}-\text{Rosser}$ 定理
    9. 主体归约引理
    10. 终止定理

    排列引理在 $\lambda_2$ 中不再成立
