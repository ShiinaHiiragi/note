...
1. 二次曲线 $F(x, y) = 0$ 与过点 $\left(x_{0}, y_{0}\right)$ 且具有方向 $X : Y$ 的直线 $\left\{\begin{array}{l}x=x_{0}+X t \\y=y_{0}+Y t\end{array}\right.$ 的交点联立得到方程
    ...
    2. $\Phi(X, Y)=0$ 时，令 $T = F_{1}\left(x_{0}, y_{0}\right) \cdot X+F_{2}\left(x_{0}, y_{0}\right) \cdot Y$
        1. $T \neq 0$，此时方程是关于 $t$ 的一次方程，有惟一的一个实根，因此直线与二次曲线有惟一的实交点
        2. $T=0$，而 $F\left(x_{0}, y_{0}\right) \neq 0$，此时方程无解，直线与二次曲线没有交点
        3. $T=F\left(x_{0}, y_{0}\right)=0$，此时方程成为一个恒等式，它能被任何值的 $t$ 所满足，因此直线全部在二次曲线上
...
3. 中心：若点 $C$ 是二次曲线的通过它的所有弦的中点，即 $C$ 是二次曲线的对称中心，则点 $C$ 称作二次曲线的中心
    ...
    2. 二次曲线按其中心的分类
        ...
        2. 非中心曲线：$I_{2}=\left|\begin{array}{ll}a_{11} & a_{12} \\ a_{12} & a_{22}\end{array}\right|=0$，即 $\dfrac{a_{11}}{a_{12}}=\dfrac{a_{12}}{a_{22}}$
            1. 无心曲线: $\dfrac{a_{11}}{a_{12}}=\dfrac{a_{12}}{a_{22}} \neq \dfrac{a_{13}}{a_{23}}$，此时曲线没有中心
            2. 线心曲线：$\dfrac{a_{11}}{a_{12}}=\dfrac{a_{12}}{a_{22}}=\dfrac{a_{13}}{a_{23}}$，此时曲线有一条中心直线
...
1. 直径：二次曲线的平行弦中点的轨迹称作这个二次曲线的直径，其所对应的平行弦称作共轭于这条直径的共轭弦，而直径也称作共轭于平行弦方向的直径
    ...
    3. 中心二次曲线的直径通过曲线的中心，无心二次曲线的直径平行于曲线的渐近方向

        线心二次曲线的直径只有一条，即曲线的中心直线
...
3. 主直径与主方向：称二次曲线垂直于其共轭弦的直径为其主直径，主直径的方向与垂直于主直径的方向都称作其主方向
    1. 主直径是二次曲线的对称轴，因此主直径也称作二次曲线的轴，轴与曲线的交点称作曲线的顶点
    2. 特征方程与特征根：方程 $\left|\begin{array}{cc}a_{11}-\lambda & a_{12} \\a_{12} & a_{22}-\lambda\end{array}\right|=0$ 或 $\lambda^{2}-I_{1} \lambda+I_{2}=0$ 称作二次曲线的特征方程，其根称作二次曲线的特征根
        1. 二次曲线的特征根都是实数且不能全为零
        2. 由二次曲线的特征根 $\lambda$ 确定的主方向 $X$ : $Y$
            1. 当 $\lambda \neq 0$ 时，为二次曲线的非渐进主方向
            2. 当 $\lambda=0$ 时，为二次曲线的渐近主方向
    3. 由二次曲线的特征方程解得两特征根为 $\dfrac{I_{1} \pm \sqrt{I_{1}^{2}-4 I_{2}}}{2}$
        ...
        2. 当二次曲线为非中心曲线时，两特征根为 $\lambda_{1}=a_{11}+a_{22}, \lambda_{2}=0$，只有一个非渐近的主方向与一条主直径
...
2. 二次曲线方程与不变量：由 $F(x, y)$ 的系数组成的一个非常数函数 $f$，若经过刚体变换
    ...
    有 $f\left(a_{11}, a_{12}, \cdots, a_{33}\right)=f\left(a_{11}^{\prime}, a_{12}^{\prime}, \cdots, a_{33}^{\prime}\right)$，则函数 $f$ 称作二次曲线在刚体变换下的不变量；若函数 $f$ 的值只是经过旋转变换
...
### 4.1.5 二次曲线的度量性质
1. 圆点与迷向直线：共轭复点 $I(1, i, 0)$ 和 $J(1,-i, 0)$ 称为圆环点，简称圆点．通过圆点的直线（无穷远直线除外）称作迷向直线
    1. 一条非退化二次曲线 ${\displaystyle S \equiv \sum_{i, j=1}^{3} a_{i j} x_{i} x_{j}=0\ \left(a_{i j}=a_{j i}, \operatorname{det}\left(a_{i j}\right) \neq 0\right)}$ 表示圆的充要条件是其通过两个圆点
    2. 仿射变换成为相似变换的充要条件是这个变换使圆点保持不变
    3. 正交变换使圆点保持不变
    4. 虚直线是迷向直线的充要条件是它上面任意两有穷点间的距离为零
    5. 一条迷向直线与另一直线的交角 $\theta$ 是不存在的或是不定的
2. $\text{Laguerre}$ 定理：设两条非迷向直线的交角为 $\theta$，又设这两条直线与过其交点的两条迷向直线所成的交比为 $\mu$，则必有 $\theta=\dfrac{1}{2 \mathrm{i}} \ln \mu$
    1. 两条非迷向直线垂直的充要条件是这两条直线与过交点的两条迷向直线调和共轭
    2. 平面上三个不共线的实有穷点确定一个圆
3. 主轴与顶点：二次曲线的一条直径如果平分一组和它垂直的弦，则此直径称作主轴，主轴与曲线的有穷交点称作顶点
    1. 抛物线有唯一主轴、唯一顶点，主轴是抛物线上无穷远点关于两个圆点的调和共轭点的极线
    2. 除圆以外的有心二次曲线只有一对主轴，它是两条渐近线所成角的平分线，顶点个数为 $4$
4. 焦点与准线：自二圆点引二次曲线的切线，其有穷交点称为二次曲线的焦点，焦点关于二次曲线的极线称为二次曲线的准线
    1. 圆的焦点即圆心，但圆心的极线是无穷远直线，因此在 $\text{Euclid}$ 平面上圆的准线不存在
    2. 抛物线有一个焦点，一条准线．焦点在主轴上，迷向切线的切点在准线上，准线垂直于主轴
    3. 对圆以外的实有心二次曲线，过圆点可作其四条切线．共有四个焦点分别在两主轴上，有四条准线，准线分别垂直于两主轴
...
1. 二次曲线：二阶曲线与二级曲线的统称
    1. 二阶曲线：射影平面上齐次坐标 $\left(x_{1}, x_{2}, x_{3}\right)$ 满足三元二次方程 ${\displaystyle \sum_{i, j=1}^{3} a_{i j} x_{i} x_{j}=0 \ \left(a_{i j}=a_{j i}\right)}$ 的点的集合称为二阶曲线．其中 $a_{i j}$ 为实数且至少有一个不为零，以上方程称为二阶曲线的方程
        ...
        2. 设有一条二阶曲线，它是由两个成射影对应的线束对应直线的交点构成的，则以这曲线上任两点为中心向曲线上的点投射直线，则可得到两个成射影对应的线束
            1. 平面内五个点，若其中任意三点都不共线，则这五个点可确定唯一一条二阶曲线
            2. 若从二阶曲线上任一点向此二阶曲线上四定点连四直线，则此四直线的交比为常数
...
2. 光滑曲线：若曲线 $\Gamma: \boldsymbol{r}(t)=x(t) \boldsymbol{e}_1+y(t) \boldsymbol{e}_2+z(t) \boldsymbol{e}_3, \ a \leqslant t \leqslant b$ 的任意分量都在 $[a, b]$ 上 $n$ 阶可导且连续，则称其为 $C^{n}$ 函数．特别地，若该曲线的任意分量都无穷阶可导，则称其为 $C^{\infty}$ 函数，则称曲线 $\boldsymbol{r}(t)$ 所确定的空间曲线为光滑曲线
    ...
    3. 曲线的弧长：${\displaystyle s= \int_{a}^{b} \left| \boldsymbol r'(t) \right| \mathrm dt = \int_{a}^{b} \sqrt{[x^{\prime}(t)]^2+[y^{\prime}(t)]^2+[z^{\prime}(t)]^2} \mathrm{d} t}$

    !!! note "正则曲线"
        设曲线 $\boldsymbol r:(a, b) \rightarrow \mathbf{R}^{2} \ (\mathbf{R}^{3})$，若 ① 曲线的每一个分量都是 $C^{\infty}$ 函数；② $\left|\dfrac{\mathrm{d} \boldsymbol r}{\mathrm{d} t}\right|>0$ 对任意 $t \in(a, b)$ 成立，则称 $r$ 为正则曲线
...
5. $\text{Green}$ 公式：设 $D$ 为平面上由（分段）光滑简单闭曲线所围的单连通闭区域，若 $P(x, y)$, $Q(x, y)$ 在 $D$ 上具有连续偏导数，则
