...
2. $\text{Backus-Naur}$ 形式句法：用 $\alpha, \beta, \gamma$ 表示公式模式（可带下标），则公式集
    ...
    3. 公式的性质
        1. $\text{var}(\alpha)$ 表示公式 $\alpha$ 中出现的所有命题变元的集合；$\alpha(p_1, p_2, \cdots, p_n)$ 表示公式 $\alpha$ 使得 $\text{var}(\alpha) \subseteq \{p_1, \cdots, p_n\}$．若 $\text{var}(\alpha) = \varnothing$，则称 $\alpha$ 为无变元公式
        ...
        3. $\text{SF}(\alpha)$ 表示公式 $\alpha$ 的子公式集合，定义为 $\left\{\begin{aligned} &\text{SF}(\alpha) = \{\alpha\}, &\alpha \in \text{At} \\& \text{SF}(\alpha \circ \beta) = \text{SF}(\alpha) \cup \text{SF}(\beta) \cup \{\alpha \circ \beta\}, &\circ \in \{\wedge, \vee, \to\}\end{aligned}\right.$
...
3. 真值函数语义学：用 $1$ 表示「真」；用 $0$ 表示「假」，集合 $2 = \{0, 1\}$ 称为真值集
    ...
    5. 真值函数：令 $\alpha$ 是公式且 $\text{var}(\alpha) = \{p_1, p_2, \cdots, p_n\}$，如果对于 $n-$元函数 $f: 2^n \to 2$ 与 $\varepsilon = (\alpha_1, \alpha_2, \cdots, \alpha_n) \in 2^n$ 都有 $f(\theta(p_1), \theta(p_2), \cdots, \theta(p_n)) = \widehat \theta(\alpha)$，其中 $\theta$ 是任意赋值使得对每个 $1\leqslant i\leqslant n$ 都有 $\theta(p_i) = a_i$，则称 $\alpha$ 定义 $n-$元真值函数 $f$
...
4. 经典命题逻辑的 $\text{Hilbert}$ 公理系统 $\textbf{HK}$
    ...
    3. 在 $\textbf{HK}$ 中，从公式集 $\Gamma$ 到公式 $\alpha$ 的一个推导是由公式组成的以 $\alpha$ 为根节点的有穷树结构 $\mathcal D$，其中每个节点满足三个条件之一：① $\gamma$ 是公理或 $\gamma \in \Gamma$；② $\gamma$ 是从子节点 $\beta$ 和 $\beta \to \gamma$ 运用规则 $\text{mp}$ 得到的；③ $\gamma$ 是从子节点 $\beta$ 利用规则 $\text{sub}$ 得到的，其中 $\beta$ 是公理
        ...
        3. 如果存在从 $\Gamma$ 到 $\alpha$ 的推导，则称公式 $\alpha$ 在 $\textbf{HK}$ 中可证；如果 $\varnothing \vdash_\textbf{HK} \alpha$，称公式在 $\textbf{HK}$ 中可证，或称 $\alpha$ 是 $\textbf{HK}$ 的定理，记作 $\vdash_\textbf{HK} \alpha$．用 $\text{Thm}(\textbf{HK})$ 表示 $\textbf{HK}$ 中所有定理的集合
...
5. 经典命题逻辑的后承演算 $\mathfrak{C}_\textbf{HK}$ 由后承公理模式和后承规则组成
    1. 后承公理模式：$\alpha, \Gamma \vDash \alpha(\mathrm{Id}); \bot, \Gamma \vdash \alpha(\bot)$
    2. 后承规则：
    ...
    3. 在 $\mathfrak C_\textbf{HK}$ 中，一个推导是由后承组成的有穷树结构
        1. 每个节点要么是后承公理，要么是使用后承规则从子节点得到的
        2. 如果存在以后承 $\Gamma \vdash \beta$ 为根节点的推导，则称该后承在 $\mathfrak C_\textbf{HK}$ 中可证，记作 $\Gamma \vdash_\textbf{HK} \beta$
    4. 以下命题在 $\mathfrak C_\textbf{HK}$ 中成立
        1. $\neg \alpha, \Gamma \vdash_\textbf{HK} \beta$ 当且仅当 $\Gamma \vdash_\textbf{HK} \alpha \vee \beta$
        2. $\alpha, \Gamma \vdash_\textbf{HK} \beta$ 当且仅当 $\Gamma \vdash_\textbf{HK} \neg \alpha \vee \beta$

6. 可靠性：如果 $\Gamma \vdash_\textbf{HK} \alpha$，则 $\Gamma \vDash \alpha$
    1. $\text{Thm}(\textbf{HK}) \subseteq \textbf{CL}$
    2. 可靠性的等价形式：如果 $\Gamma$ 是可满足的，则 $\Gamma$ 是 $\textbf{HK}-$一致的
7. 一致性：$\textbf{HK}$ 是一致的
    1. 如果公式集 $\Gamma$ 有 $\Gamma \not \vdash_\textbf{HK} \bot$，则称 $\Gamma$ 是 $\textbf{HK}-$一致的，否则 $\Gamma$ 是 $\textbf{HK}-$不一致的
    2. 如果公式集 $\Gamma$ 是 $\textbf{HK}-$一致的，且对任何 $\textbf{HK}-$一致的公式集 $\Delta$，如果 $\Gamma \subseteq \Delta$，那么 $\Gamma = \Delta$，则称 $\Gamma$ 是极大 $\textbf{HK}-$一致的
    3. 对任何极大 $\textbf{HK}-$一致公式集 $\Gamma$
        1. 如果 $\Gamma \vdash_\textbf{HK} \alpha$，则 $\alpha \in \Gamma$
        2. $\text{Thm}(\textbf{HK}) \subseteq \Gamma$ 且 $\bot \notin \Gamma$
    ...
    4. 如果 $\Gamma$ 是$\textbf{HK}-$一致的公式集，那么存在极大 $\textbf{HK}-$一致的公式集 $\Delta$ 使得 $\Gamma \subseteq \Delta$
8. 完全性：如果 $\Gamma \vDash \alpha$，那么 $\Gamma \vdash_\textbf{HK} \alpha$
    1. $\textbf{CL} \subseteq \text{Thm}(\textbf{HK})$
    2. 完全性的等价形式：如果 $\Gamma$ 是 $\textbf{HK}-$一致的，则 $\Gamma$ 是可满足的
    3. 紧致性：一个公式集 $\Gamma$ 可满足当且仅当 $\Gamma$ 的每个有穷子集可满足
9. 其他性质
    1. $\text{Post}$ 完全性：$\textbf{HK}$ 是 $\text{Post}$ 完全的，即如果 $\not \vdash_\textbf{HK} \alpha$，那么在系统 $\textbf{HK}$ 上增加 $\alpha$ 作为公理所得到的系统是不一致的
    2. 可判定性：$\textbf{CL}$ 是可判定的，即存在一种能行的方法使得对任何公式 $\alpha$ 可在有穷步骤之内确定 $\alpha$ 是否属于 $\textbf{CL}$，因此 $\text{Thm}(\textbf{HK})$ 是可判定的
    3. 析取性质：$\textbf{HK}$ 不具有析取性质，即存在公式 $\alpha \vee \beta$，有 $\vdash_\textbf{HK} \alpha \vee \beta$ 且 $\not \vdash_\textbf{HK} \alpha$ 且 $\not \vdash_\textbf{HK} \beta$
    4. 插值性质：$\textbf{HK}$ 具有插值性质，即对任何公式 $\alpha \to \beta$，如果 $\vdash_\textbf{HK} \alpha \to \beta$，那么存在公式 $\gamma$ 使得 ① $\vdash_\textbf{HK} \alpha \to \chi$；② $\vdash_\textbf{HK} \chi \to \beta$；③ （变元条件）$\text{var}(\chi) \subseteq \text{var}(\alpha) \subseteq \text{var}(\beta)$；公式$\chi$ 称为 $\alpha$ 与 $\beta$ 的插值

        !!! note "证明插值性质的引理"
            如果 $\text{var}(\alpha) \cap \text{var}(\beta) = \varnothing$，那么 $\vDash \alpha \to \beta$ 当且仅当 $\vDash \neg \alpha$ 或 $\vDash \beta$
