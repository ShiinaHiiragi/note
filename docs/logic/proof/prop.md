...
4. 经典命题逻辑的 $\text{Hilbert}$ 公理系统 $\mathbf{HK}$
    ...
    3. 在 $\mathbf{HK}$ 中，从公式集 $\Gamma$ 到公式 $\alpha$ 的一个推导是由公式组成的以 $\alpha$ 为根节点的有穷树结构 $\mathcal D$，其中每个节点满足三个条件之一：① $\gamma$ 是公理或 $\gamma \in \Gamma$；② $\gamma$ 是从子节点 $\beta$ 和 $\beta \to \gamma$ 运用规则 $\text{mp}$ 得到的；③ $\gamma$ 是从子节点 $\beta$ 利用规则 $\text{sub}$ 得到的，其中 $\beta$ 是公理
        ...
        2. 在 $\mathbf{HK}$ 中，如果存在公式集 $\Gamma$ 到公式 $\alpha$ 的推导，则称 $\alpha$ 是 $\Gamma$ 的句法后承，记作 $\Gamma \vdash_\mathbf{HK} \alpha$，不引起歧义时，下标 $\mathbf{HK}$ 可删除
        3. 如果存在从 $\Gamma$ 到 $\alpha$ 的推导，则称公式 $\alpha$ 在 $\mathbf{HK}$ 中可证；如果 $\varnothing \vdash_\mathbf{HK} \alpha$，称公式在 $\mathbf{HK}$ 中可证，或称 $\alpha$ 是 $\mathbf{HK}$ 的定理，记作 $\vdash_\mathbf{HK} \alpha$．用 $\mathrm{Thm}(\mathbf{HK})$ 表示 $\mathbf{HK}$ 中所有定理的集合
    4. 经典命题逻辑的 $\text{Hilbert}$ 公理系统元定理
        ...
        3. 如果 $\Gamma \vdash_\mathbf{HK} \alpha$ 且 $\Gamma \vdash_\mathbf{HK} \beta$，那么 $\Gamma \vdash_\mathbf{HK} \alpha \wedge \beta$
        4. 如果 $\alpha, \beta, \Gamma \vdash_\mathbf{HK} \gamma$，那么 $\alpha \wedge \beta, \Gamma \vdash_\mathbf{HK} \gamma$
        5. 如果 $\Gamma \vdash_\mathbf{HK} \alpha$ 或 $\Gamma \vdash_\mathbf{HK} \beta$，那么 $\Gamma \vdash_\mathbf{HK} \alpha \vee \beta$
        6. 如果 $\alpha, \Gamma \vdash_\mathbf{HK} \gamma$ 且 $\beta, \Gamma \vdash_\mathbf{HK} \gamma$，那么 $\alpha \vee \beta, \Gamma \vdash_\mathbf{HK} \gamma$
        7. 如果 $\Gamma \vdash_\mathbf{HK} \alpha$ 且 $\beta, \Delta \vdash_\mathbf{HK} \gamma$，那么 $\alpha \to \beta, \Gamma, \Delta \vdash_\mathbf{HK} \gamma$
        8. 如果 $\Gamma \vdash_\mathbf{HK} \alpha$ 且 $\Gamma \vdash_\mathbf{HK} \alpha \to \beta$，那么 $\Gamma \vdash_\mathbf{HK} \beta$
        9. 弱化规则：如果 $\Gamma \vdash_\mathbf{HK} \alpha$ 且 $\Gamma \subseteq \Delta$，那么 $\Delta \vdash_\mathbf{HK} \alpha$
        10. 切割规则：如果 $\Gamma \vdash_\mathbf{HK} \alpha$ 且 $\alpha, \Gamma \subseteq \beta$，那么 $\Gamma, \Delta \vdash_\mathbf{HK} \beta$
        11. 演绎定理：对任意公式集 $\Gamma \cup \{\alpha, \beta\}$，$\alpha, \Gamma \vdash_\mathbf{HK} \beta$ 当且仅当 $\Gamma \vdash_\mathbf{HK} \alpha \to \beta$
        12. 对任意公式集 $\Gamma \cup \{\alpha, \beta\}$，如果 $\alpha, \Gamma \vdash_\mathbf{HK} \beta$ 并且 $\neg \alpha, \Gamma \vdash_\mathbf{HK} \beta$，那么 $\Gamma \vdash_\mathbf{HK} \beta$
...
6. 可靠性：如果 $\Gamma \vdash_\mathbf{HK} \alpha$，则 $\Gamma \vDash \alpha$
...
7. 一致性：$\mathbf{HK}$ 是一致的
    ...
    4. 如果 $\Gamma$ 是$\mathbf{HK}-$一致的公式集，那么存在极大 $\mathbf{HK}-$一致的公式集 $\Delta$ 使得 $\Gamma \subseteq \Delta$
8. 完全性：如果 $\Gamma \vDash \alpha$，那么 $\Gamma \vdash_\mathbf{HK} \alpha$
...
9.  其他性质
    ...
    4. 插值性质：$\mathbf{HK}$ 具有插值性质，即对任何公式 $\alpha \to \beta$，如果 $\vdash_\mathbf{HK} \alpha \to \beta$，那么存在公式 $\gamma$ 使得 ① $\vdash_\mathbf{HK} \alpha \to \chi$；② $\vdash_\mathbf{HK} \chi \to \beta$；③ （变元条件）$\mathrm{var}(\chi) \subseteq \mathrm{var}(\alpha) \subseteq \mathrm{var}(\beta)$；公式$\chi$ 称为 $\alpha$ 与 $\beta$ 的插值
        ...
        !!! note "证明插值性质的引理"
            如果 $\mathrm{var}(\alpha) \cap \mathrm{var}(\beta) = \varnothing$，那么 $\vDash \alpha \to \beta$ 当且仅当 $\vDash \neg \alpha$ 或 $\vDash \beta$
