...
4. 将结果写成分数形式：$[(p,q)]=\dfrac{[(p,1)]}{[(q,1)]}$，于是有 $Q=\left\{\dfrac{[(p,1)]}{[(q,1)]}|p,q\in Z\wedge q\neq 0\right\}$．把所有形如 $[(p,1)]$ 的元素写成 $p$，称之为新整数．于是 $Z\subset Q=\left\{\dfrac{p}{q}|p,q\in Z\wedge q\neq 0\right\}$，将 $\mathbb{Q}$ 称为有理数集
